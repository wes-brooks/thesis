...
variable \citep{Cleveland-Grosse-1991,Hastie-Tibshirani-1993}. Here
...
proposed LAGR method is to use the adaptive group Lasso for local
variable selection and coefficient estimation in a locally linear
...
to the Boston house price dataset. In Section \ref{sec:lagr-gllm},
...
The local log-likelihood at $\bm{s}$ is maximized to obtain an estimate
$\tilde{\bm{\zeta}}(\bm{s})$ of the local coefficients. Let $\bm{W}\!(\bm{s})={\rm diag}\left\{ K_{h}(\|\bm{s}-\bm{s}_{i}\|)\right\} _{i'=1}^{n}$
...
\end{array}\right)$. Let $\kappa_{0}=\int_{R^{2}}K(\|\bm{s}\|)ds$, $\kappa_{2}=\int_{R^{2}}[(1,0)\bm{s}]^{2}K(\|\bm{s}\|)ds=\int_{R^{2}}[(0,1)\bm{s}]^{2}K(\|\bm{s}\|)ds$,
...
vectors that are independent of $\varepsilon_{1},\dots,\varepsilon_{n}$.
Also $\Psi(\bm{s})=E\left\{ \bm{X}(\bm{s})\bm{X}(\bm{s})^{T}|\bm{s}\right\} $
and $\Psi_{(a)}(\bm{s})=E\left\{ \bm{X}_{(a)}(\bm{s})\bm{X}_{(a)}(\bm{s})^{T}|\bm{s}\right\} $
are positive-definite and differentiable at location $\bm{s}$, $E\left|\bm{X}(\bm{s})\right|^{2q}<\infty$,
and $E\left|\varepsilon(\bm{s})\right|^{2q}<\infty$ for some $q>2$.
\item[(A.4)] The coefficient functions $ $$\beta_{j}(\cdot)$, $j=1,\dots,p$
have continuous second partial derivatives at $\bm{s}$.
\item[(A.6)] The function $f(\bm{s})$ is differentiable at $\bm{s}$ and $f(\bm{s})>0$.
...
\end{itemize}
These conditions are generally equivalent to those of \citet{Cai-Fan-Li-2000}
and \citet{Sun-Yan-Zhang-Lu-2014}. The covariates $\bm{X}(\bm{s}_{1}),\dots,\bm{X}(\bm{s}_{n})$
were assumed to be $iid$ in \citet{Sun-Yan-Zhang-Lu-2014}, which
is not required here. The existence of of $\Psi(\cdot)$ is necessary
for the existence of the limiting distribution of $\hat{\bm{\beta}}(\bm{s})$;
its differentiability and that of $f(\cdot)$ are necessary in order
to approximate those functions by Taylor expansion. The condition
on moments of $Y$ is used when bounding the remainder term from Taylor
expansion. 
\begin{thm}[Asymptotic normality]
\label{theorem:normality}  Under (A.1)-(A.8), if $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$
...
\label{theorem:selection} Under (A.1)-(A.8), if $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$
and $hn^{-1/2}b_{n}\xrightarrow{p}\infty$, then 
\[
...
the true nonzero coefficients are known in advance \citep{Sun-Yan-Zhang-Lu-2014},
and that the LAGR estimates of true zero coefficients tend to zero
with probability one. Thus, selection and estimation by LAGR has the
oracle property. To establish the oracle properties of LAGR, we assumed
that $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$ and $hn^{-1/2}b_{n}\xrightarrow{p}\infty$.
Therefore, $h^{-1}n^{-1/2}\lambda_{n}\to0$ for $j\le p_{0}$ and
$hn^{-1/2}\lambda_{n}\|\bm{\zeta}_{(j)}(\bm{s})\|^{-\gamma}\to\infty$
...
the ``step'' function $\mathbf{\beta_{step}(\bm{s})=}1$ if $s_{x}>0.6$,
$5s_{x}-2$ if $0.4<s_{x}\le0.6$, and $0$ otherwise. The second
is the gradient function, $\beta_{gradient}(\bm{s})=s_{x}$, and the
third is the parabola $\beta_{parabola}(\bm{s})=1-2\left\{ (s_{x}-0.5)^{2}+(s_{y}-0.5)^{2}\right\} $.
...
The MISE of the estimates of $\beta_{1}(\bm{s})$ are in Table \ref{tab:mise}.
...
where $\phi$ and $\theta$ are parameters, $E\left\{ y(\bm{s})|\bm{x}(\bm{s})\right\} =\mu(\bm{s})=b'\left(\theta(\bm{s})\right)$,
$\theta(\bm{s})=(g\circ b')^{-1}\left(\eta(\bm{s})\right)$, $\eta(\bm{s})=\bm{x}(\bm{s})^{T}\bm{\beta}(\bm{s})=g\left(\mu(\bm{s})\right)$,
and $\text{\text{Var}}\left\{ y(\bm{s})|\bm{x}(\bm{s})\right\} =b''\left(\theta(\bm{s})\right)$.
...
The local quasi-likelihood is convex, and is defined in terms of its
derivative, the local quasi-score function $\left(\partial/\partial\mu\right)Q(\mu,y)=(y-\mu)\{V(\mu)\}^{-1}$.
...
and $\Gamma_{(a)}\left(\bm{s}\right)=E\left\{ \rho\left(\bm{s},\bm{X}_{(a)}(\bm{s})\right)\bm{X}_{(a)}(\bm{s})\bm{X}_{(a)}(\bm{s})^{T}|\bm{s},\bm{Z}(\bm{s})=\bm{z}\right\} $.
Assume the following conditions:
\begin{itemize}
\item[(A.9)] The functions $g'''\left(\bm{s}\right)$, $\nabla\Gamma\left(\bm{s}\right)$,
$\nabla\Gamma_{(a)}\left(\bm{s}\right)$, $V\left(\mu\left(\bm{s},\bm{z}\right)\right)$,
...
\end{itemize}
These additonal conditions are necessary for Taylor expansion of the
local quasi-likelihood. Condition (A.10) assures that the local quasi-likelihood
is convex and has a unique minimizer.
\begin{thm}[Asymptotic normality]
\label{theorem:normality-glm}  Under (A.1)-(A.10), if $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$
and $hn^{-1/2}b_{n}\xrightarrow{p}\infty$, then 
\begin{gather*}
...
\begin{thm}[Selection consistency]
\label{theorem:selection-glm}  Under (A.1)-(A.10), if $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$
and $hn^{-1/2}b_{n}\xrightarrow{p}\infty$ then 
\[
P\left\{ \|\hat{\bm{\zeta}}_{(j)}(\bm{s})\|=\bm{0}\right\} \to1\text{ if }j>p_{0}.
\]
...
is that $\sigma^{2}\Psi_{(a)}(\bm{s})^{-1}$ in the variance term
of Theorem (\ref{theorem:normality}) has been replaced by $\Gamma_{(a)}(\bm{s})^{-1}$
in Theorem (\ref{theorem:normality-glm}) because the variance of
the response in the GLM case depends on the expectation of the response.
...
setting: true nonzero coefficients are dropped from the model with
probability going to one. Thus, the oracle properties for the GLM
setting are esatblished.
