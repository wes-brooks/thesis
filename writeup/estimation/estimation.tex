...
We have shown that the method of LAGR has oracle properties for local
variable selection and coefficient estimation in VCR models. This
is a novel result; the existing literature on variable selection for
VCR models focusses on global variable selection. The method of LAGR
extends the adaptive group lasso (AGL) - in particular, the previous
literature on the AGL is insufficient for local selection in a VCR
model because the local weights are functions of the kernel $K(\cdot)$
and the bandwidth $h$, with the result that the local observation
weights change with sample size and the coefficient estimates converge
at a slower rate than in the traditional AGL model. As a consequence,
the conditions for oracle properties of the AGL must be refined for
the LAGR setting (compare (A.8) to the corresponding condition of
\citet{Wang-Leng-2008}). Satisfying (A.8) required an additional
restriction on $\gamma$, the unpenalized group norm in the LAGR penalty.
