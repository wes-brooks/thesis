...
for (h in hh) {
    ...
    for (i in 1:nrow(sim)) {
        bootstrap.coefs = mvrnorm(B, mu=model[['model']][[i]][['model']][['LS.coefs']][1:5], Sigma=4/3/pi * model[['model']][[i]][['model']][['results']][['full.model.cov']][1:5,1:5])
    ...
    #Following the notation of Efron (2014):
    V = 4/3/pi * model[['model']][[i]][['model']][['results']][['full.model.cov']][1:5,1:5]
    cov(t(coef.distribution), cc[[1]])
}
