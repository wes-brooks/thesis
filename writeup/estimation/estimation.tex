...
\item[(A.4)] The coefficient functions $ $$\beta_{j}(\cdot)$ for $j=1,\dots,p$
have continuous second-order partial derivatives at $\bm{s}$.
\item[(A.6)] The density function $f(\bm{s})$ of the sampling locations is differentiable
at $\bm{s}$ and $f(\bm{s})>0$.
\item[(A.6)] $E\left\{ \left|\bm{X}(\bm{s})\right|^{3}|\bm{s}\right\} $ and $E\left\{ Y(\bm{s})^{4}|\bm{X}(\bm{s}),\bm{s}\right\} $
are continuous at a given location $\bm{s}$.
\item[(A.7)] $h=O\left(n^{-1/6}\right)$
\end{itemize}
These regularity conditions are generally similar to those of \citet{Cai-Fan-Li-2000}
and \citet{Sun-Yan-Zhang-Lu-2014}. However, the covariates $\bm{X}(\bm{s}_{1}),\dots,\bm{X}(\bm{s}_{n})$
were assumed to be $iid$ in \citet{Sun-Yan-Zhang-Lu-2014}, which
is not required here. The existence of of $\bm{\Psi}(\cdot)$ is needed
for the existence of the limiting distribution of $\hat{\bm{\beta}}(\bm{s})$;
its differentiability and that of $f(\cdot)$ are used to approximate
those functions by Taylor's expansions. The condition on the moments
of $Y$ is used when bounding the remainder term in the Taylor's expansions.
\begin{thm}[Asymptotic normality]
\label{theorem:normality}  Under (A.1)--(A.8), if $h^{-1}n^{-1/2}a_{n}\xrightarrow{p}0$
and $hn^{-1/2}b_{n}\xrightarrow{p}\infty$, then it follows that
\begin{gather*}
...
for $\gamma>1$. The technical proof of theorems \ref{theorem:normality}
...
coefficient estimate. The coefficient of the CRIM variable was estimated
...
and $\bm{\Gamma}_{(a)}\left(\bm{s}\right)=E\left\{ \rho\left(\bm{s},\bm{X}_{(a)}(\bm{s})\right)\bm{X}_{(a)}(\bm{s})\bm{X}_{(a)}(\bm{s})^{T}|\bm{s},\bm{Z}(\bm{s})=\bm{z}\right\} $.
Assume the following regularity conditions:
\begin{itemize}
