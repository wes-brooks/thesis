...
At location $\bm{s}$, let there be are $p_{0}(\bm{s})<p$ covariates $\bm{X}_{(a)}(\bm{s})$
...
\begin{align*}
\hat{{\rm df}}(\lambda_{n};\bm{s})= & \sum_{j=1}^{p}I\left(\|\hat{\bm{\zeta}}(\lambda_{n};\bm{s})\|>0\right)+\sum_{j=1}^{p}\|\hat{\bm{\zeta}}(\lambda_{n};\bm{s})\|\|\tilde{\bm{\zeta}}(\bm{s})\|^{-1}(d-1)\\
...
In no case was the MISE for LAGR more than $11\%$ greater than for VCR, while the MISE for estimating $\beta_4$ with $\rho=0.9$, $\sigma_{\varepsilon}=0.5$, and $n=100$ setting was $140$ times greater for VCR than for LAGR.
That was the exception, as under the other simulation settings the greatest improvement for LAGR over VCR tended to be a $2-3$ times reduction in MISE.
...
The estimated coefficients of CRIM and LSTAT were everywhere negative or exactly zero, meaning that the crime rate and proportion of ``lower-status" individuals were associated with a lower median house price. 
