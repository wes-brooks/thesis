...
The next proofs require the following lemmas. Define the $q$-functions
to be the derivatives of the quasi-likelihood: $q_{j}(t,y)=\left(\partial/\partial t\right)^{j}Q\left(g^{-1}\left(t\right),y\right)$.
...
\begin{lem}[\label{lemma:omega-expectation}]
\[
...
\begin{lem}[\label{lemma:omega-variance}]
\[
...
\begin{lem}[\label{lemma:delta-expectation}]
\[
...
\begin{lem}[\label{lemma:delta-variance}]
\[
...
which is establised via the Cram\textipa{\'e}r-Wold device. Let $\bm{d}\in\mathbb{R}^{3p}$
...
From Lemma \ref{lemma:omega-expectation} and Lemma \ref{lemma:omega-variance},
the right hand side of (\ref{eq:glm-selection}) is $O_{p}\left(1\right)$,
so for $\hat{\bm{\zeta}}_{p}\left(\bm{s}\right)$ to be a solution,
we must have that $hn^{-1/2}\phi_{p}\left(\bm{s}\right)\hat{\bm{\zeta}}_{p}\left(\bm{s}\right)/\|\hat{\bm{\zeta}}_{p}\left(\bm{s}\right)\|=O_{p}\left(1\right)$.
