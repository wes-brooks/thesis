...
where $\tau_{\beta}=1$ is the range parameter. The scale of the coefficient surface $\beta_j(\bm{s})$ was set via the variance $\sigma_j^2$, and the values used in the simulations were $\bm{\sigma}^2 = \left( 10, 1, 0.1 \right)$.
...
For both estimation methods, the bandwidth parameter was $h=(3/2)n^{-1/6} - 0.36$.

The results are presented in terms of the mean integrated squared
error (MISE) of the coefficient surface estimates $\hat{\beta}_{1}(\bm{s}),\dots,\hat{\beta}_4(\bm{s})$,
the MISE of the fitted response $\hat{y}(\bm{s})$, and the frequency
with which the coefficient surface estimates $\hat{\beta}_{1}(\bm{s}),\dots,\hat{\beta}_4(\bm{s})$
...
% Fri Sep 19 13:04:18 2014
  \multirow{6}{*}{100} & \multirow{2}{*}{0} & 0.5 & \textbf{1.43} & 1.46 & \textbf{0.42} & 0.43 & \textbf{0.03} & 0.10 & \textbf{0.05} & 0.09 \\ 
    &  & 1.0 & \textbf{1.79} & 1.91 & \textbf{0.55} & 0.55 & \textbf{0.32} & 0.40 & \textbf{0.13} & 0.21 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & \textbf{1.78} & 1.82 & \textbf{0.29} & 0.33 & \textbf{0.10} & 0.14 & \textbf{0.35} & 0.51 \\ 
    &  & 1.0 & 1.34 & \textbf{1.30} & 0.42 & \textbf{0.31} & \textbf{0.05} & 0.07 & \textbf{0.20} & 0.45 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & \textbf{2.73} & 3.62 & \textbf{1.26} & 2.73 & \textbf{1.39} & 2.20 & \textbf{1.67} & 2.27 \\ 
    &  & 1.0 & \textbf{2.08} & 2.71 & \textbf{0.77} & 1.31 & \textbf{0.22} & 0.65 & \textbf{0.74} & 1.89 \\ 
   \hline \multirow{6}{*}{200} & \multirow{2}{*}{0} & 0.5 & \textbf{1.32} & 1.33 & 0.18 & \textbf{0.17} & \textbf{0.15} & 0.24 & \textbf{0.04} & 0.07 \\ 
    &  & 1.0 & \textbf{1.24} & 1.25 & 0.44 & \textbf{0.32} & \textbf{0.04} & 0.09 & \textbf{0.05} & 0.19 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & \textbf{1.41} & 1.56 & 0.20 & \textbf{0.19} & \textbf{0.07} & 0.15 & \textbf{0.04} & 0.10 \\ 
    &  & 1.0 & \textbf{1.24} & 1.34 & 0.38 & \textbf{0.34} & 0.04 & \textbf{0.04} & \textbf{0.04} & 0.12 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & \textbf{2.32} & 2.53 & \textbf{1.12} & 1.15 & \textbf{0.67} & 0.90 & \textbf{0.64} & 0.78 \\ 
    &  & 1.0 & \textbf{2.19} & 2.22 & \textbf{0.81} & 1.31 & \textbf{0.12} & 0.38 & \textbf{0.15} & 0.63 \\ 
   \hline \multirow{6}{*}{400} & \multirow{2}{*}{0} & 0.5 & 1.23 & \textbf{1.23} & 0.18 & \textbf{0.17} & \textbf{0.06} & 0.08 & \textbf{0.04} & 0.05 \\ 
    &  & 1.0 & 1.14 & \textbf{1.12} & 0.19 & \textbf{0.18} & \textbf{0.06} & 0.07 & \textbf{0.02} & 0.06 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & 1.27 & \textbf{1.26} & \textbf{0.19} & 0.24 & \textbf{0.06} & 0.08 & \textbf{0.01} & 0.09 \\ 
    &  & 1.0 & 1.20 & \textbf{1.20} & 0.35 & \textbf{0.29} & \textbf{0.06} & 0.08 & \textbf{0.03} & 0.07 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & \textbf{1.61} & 1.62 & \textbf{0.41} & 0.41 & \textbf{0.36} & 0.54 & \textbf{0.47} & 0.61 \\ 
    &  & 1.0 & \textbf{1.34} & 1.70 & \textbf{0.58} & 0.71 & \textbf{0.17} & 0.34 & \textbf{0.03} & 0.23 \\

  
	\hline
	\end{tabular}
	\caption{For each setting as a combination of sample size $n$, cross-covariate correlation $\rho$, and error standard deviation $\sigma_{\varepsilon}$, the mean integrated squared error (MISE) of the coefficient estimates. The MISE of $\hat{\beta}_1, \dots, \hat{\beta}_4$ from estimation by local adaptive grouped regularization (LAGR) is compared to that from estimation by locally linear regression without selection (VCR). \textbf{Highlighting} indicates the smallest MISE for $\hat{\beta}_1, \dots, \hat{\beta}_4$.}
	\label{tab:mise}
\end{table}
...
The improvement by MISE of LAGR over VCR was greater for covariates with smaller influence, with LAGR producing the smaller MISE for $\beta_3$ or $\beta_4$ $35$ times out of a possible $36$.
With other factors held constant, the MISE for estimating the coefficients tended to decrease as the sample size increased, and to to increase as the error variance or the correlation between covariates increased. 
There were 
...
% latex table generated in R 3.1.1 by xtable 1.7-3 package
% Fri Sep 19 13:05:26 2014
  \multirow{6}{*}{100} & \multirow{2}{*}{0} & 0.5 & 0.00 & 0.33 & 0.56 & 0.88 \\ 
    &  & 1.0 & 0.00 & 0.24 & 0.34 & 0.38 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & 0.00 & 0.22 & 0.72 & 0.06 \\ 
    &  & 1.0 & 0.00 & 0.75 & 0.88 & 0.62 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & 0.24 & 0.48 & 0.41 & 0.28 \\ 
    &  & 1.0 & 0.06 & 0.75 & 0.54 & 0.69 \\ 
   \hline \multirow{6}{*}{200} & \multirow{2}{*}{0} & 0.5 & 0.00 & 0.12 & 0.50 & 0.78 \\ 
    &  & 1.0 & 0.00 & 0.88 & 0.96 & 0.72 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & 0.00 & 0.34 & 0.74 & 0.68 \\ 
    &  & 1.0 & 0.00 & 0.56 & 0.99 & 0.83 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & 0.07 & 0.25 & 0.36 & 0.22 \\ 
    &  & 1.0 & 0.04 & 0.04 & 0.80 & 0.76 \\ 
   \hline \multirow{6}{*}{400} & \multirow{2}{*}{0} & 0.5 & 0.00 & 0.05 & 0.44 & 0.57 \\ 
    &  & 1.0 & 0.00 & 0.21 & 0.55 & 0.72 \\ 
    & \multirow{2}{*}{0.5} & 0.5 & 0.00 & 0.14 & 0.79 & 0.83 \\ 
    &  & 1.0 & 0.00 & 0.32 & 0.70 & 0.81 \\ 
    & \multirow{2}{*}{0.9} & 0.5 & 0.00 & 0.46 & 0.49 & 0.34 \\ 
    &  & 1.0 & 0.00 & 0.27 & 0.73 & 0.86 \\ 
...
In general, the variables with less influence in the true simulated data were more likely to be 
...
\caption{Coefficients for the Boston house price data as estimated by local adaptive grouped regularization.
...
\begin{table}
    ...
    \hline
    CRIM, RM, RAD, TAX, LSTAT & 471 \\
    CRIM, RM, RAD, LSTAT & 43 \\
    RM, RAD, LSTAT & 6 \\
    RM, LSTAT & 1 \\
    \hline
    \end{tabular}
...
CRIM & -0.15 & 0.07 & 0.01 \\ 
  ...
  RAD & 0.21 & 0.25 & 0.00 \\ 
  TAX & -0.02 & 0.01 & 0.10 \\ 
  LSTAT & -0.73 & 0.13 & 0.00 \\ 
   \hline
\end{tabular}
\caption{The mean, standard deviation, and proportion of zeros among the estimates of the local coefficients in a model for the median house price in census tracts in Boston, with coefficients selected and fitted by local adaptive grouped regularizatcapt The covariates are CRIM (per capita crime rate in the census tract), RM (average number of rooms per home sold in the census tract), RAD (an index of the tract's access to radial roads), TAX (property tax per USD10,000 of property value), and LSTAT (percentage of the tract's residents who are considered ``lower status").} 
\label{tab:boston-coefs-lagr}
\end{table}

One interesting result from the simulation is an apparent relationship between access to radial roads and the coefficient of the TAX variable.
Of the 506 Boston census tracts, there were 132 with the highest possible index for access to radial roads, and 50 where the estimated coefficient of the TAX variable was zero.
Only one tract had the highest RAD index and a zero coefficient for the TAX variable.
This is a quite extreme result against a null hypothesis that a zero coefficient for the TAX variable is independent of the whether a tract has the highest access to radial roads.
If the tracts are treated as independent observations, then Pearson's chi-squared test tells us that the p-value of such a null hypothesis is $4.4 \times 10^{-5}$, which is well below any reasonable standard for rejection.
This kind of result could elicit further inquiry into the reasons for the apparent relationship.
For instance, it might be the case that the property tax rate has a slight influence on house prices.
If the radial roads facilitate easier travel, then they may increase the number of people who are able to consider buying a house in a low-tax jurisdiction.
Meanwhile, the preference for lower tax rates may be weak enough that it doesn't affect house prices where travel is more difficult.
