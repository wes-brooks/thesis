...
Let $\bm{Z}_{i}=\left\{ \bm{Z}(\bm{s}_i)\right\} _{i}$ denote the
$i$th row of the matrix $\bm{Z}(\bm{s})$ as a column vector.
...
\begin{align}
\ell\left(\bm{\zeta}\right)= & -(1/2)\sum_{i=1}^{n}\left[\log{\sigma^{2}}+\sigma^{-2}\left\{ y_{i}- \{\bm{z}(\bm{s}_i)\}_i \bm{\zeta}(\bm{s}_i)\right\} ^{2}\right].\label{eq:coefficients}
\end{align}
