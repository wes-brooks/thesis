...
is positive-definite and differentiable at location $\bm{s}$, $E\left|\bm{X}(\bm{s})\right|^{2q}<\infty$,
...
the quasi-score function $\left(\partial/\partial\mu\right)Q(\mu,y)=(y-\mu)\{V(\mu)\}^{-1}$.
...
\ref{lemma:omega} and \ref{lemma:delta} that the distribution of
