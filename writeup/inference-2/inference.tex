...
Varying coefficient regression (VCR) is a data analysis method that relates the response $Y$ to its covariates $\bm{X}$ through a linear equation of coefficients $\bm{\beta}$. The coefficients in a VCR model vary over the model's domain $\mathcal{S}$, which is often a region of time or space. In the context of a generalized linear model (GLM) with link function $g$, the relationship is
\begin{equation}
EY_i = g^{-1} \left( \sum_{j=1}^p X_{ij} \beta_j(s_i) \right)
\end{equation}
where $s \in \mathcal{S}$ indexes the model's domain and $p$ is the number of covariates.
